...
entity vga is
   PORT (
      ...
      signal line_req_out  : out std_logic;  -- 请求下一行数据
      signal line_num_out  : out integer range 0 to 479;  -- 当前行号
...
signal line_num : integer range 0 to 479 := 0;
------
...
process(pixel_valid,tvalid_in,count_r,tdata_in,tdata_r,hcount_r)
   variable x : unsigned(hcount_r'range);
begin
   if(pixel_valid='0') then
      ...
      x := hcount_r - to_unsigned(H_FP,hcount_r'length);
         if(x < to_unsigned(80,x'length)) then
            ...
            VGA_G <= (others=>'1');
            VGA_B <= (others=>'0');
         elsif(x < to_unsigned(240,x'length)) then
            VGA_R <= (others=>'0');
            VGA_G <= (others=>'1');
            VGA_B <= (others=>'1');
            ...
            VGA_G <= (others=>'1');
            VGA_B <= (others=>'0');
         elsif(x < to_unsigned(400,x'length)) then
            VGA_R <= (others=>'1');
            VGA_G <= (others=>'0');
            VGA_B <= (others=>'1');
            ...
            VGA_G <= (others=>'0');
            VGA_B <= (others=>'0');
         elsif(x < to_unsigned(560,x'length)) then
            VGA_R <= (others=>'0');
            VGA_G <= (others=>'0');
            VGA_B <= (others=>'1');
         else
